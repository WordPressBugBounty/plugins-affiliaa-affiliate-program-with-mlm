(function ($) {
	...
	$(function () {
		...
		$(document).find(".rtwalwm_custom_banner_table").DataTable({
			responsive: true,
			order: [],
			columnDefs: [
				{ orderable: false, targets: [0] },
		...
		$(document).on('click', '.rtwalwm_generate_csv_affiliate', function () {
			var rtwalwm_data = {
				action: 'rtwalwm_generate_csv_affiliate',
				rtwalwm_security_check: rtwalwm_global_params.rtwalwm_nonce
			};
		...
		$(document).on('change', '.rtwalwm_switch', function(){
			$.blockUI({ message: '' });
			setTimeout(() => {
				$.unblockUI();
			}, 500);
