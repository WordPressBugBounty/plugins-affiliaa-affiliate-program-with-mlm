...
if( isset( $_GET[ 'rtwalwm_tab' ] ) && $_GET[ 'rtwalwm_tab' ] == 'affiliate_tools' ){
	...
	$rtwalwm_user_name_new = $rtwalwm_user_name->data->user_login;


	// update code start here 

	$rtwalwm_curr_aff_id = get_current_user_id();
	$rtwalwm_aff_custom_code = get_user_meta( $rtwalwm_curr_aff_id, 'rtwalwm_referee_custom_str', true );
	if(!$rtwalwm_aff_custom_code){

		$randomString = $this->rtwalwm_generate_custom_code(6);

		update_user_meta( $rtwalwm_curr_aff_id, 'rtwalwm_referee_custom_str', $randomString );
	...
	$rtwalwm_all_users = get_users(array(
		...
		'meta_value'   => $rtwalwm_aff_custom_code,
	));
...
if( isset( $_GET[ 'rtwalwm_tab' ] ) && $_GET[ 'rtwalwm_tab' ] == 'custom_banner' ){
	...
	if(	$rtwalwm_custom_banner != '' )
	{
		...
		foreach($rtwalwm_custom_banner as $key => $value)
		{
			...
			$rtwalwm_image_width = $value['image_width'];
