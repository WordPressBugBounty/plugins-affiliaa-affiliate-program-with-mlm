...
class Rtwalwm_Wp_Wc_Affiliate_Program_Public {
	...
	*/
	function rtwalwm_url_check(){
		...
		if( isset( $_GET[ 'rtwalwm_aff' ] ) ){
			...
			if(isset($_GET['action'])){
				$rtwalwm_aff_share = $_GET['action'];
			}
			else{
				$rtwalwm_aff_share = 0;
			}

			$custom_str = $_GET[ 'rtwalwm_aff' ];
			$rtwalwm_referee_aff = get_users(array(
				'meta_key' => 'rtwalwm_referee_custom_str',
				'meta_value' => $custom_str
			));

			if($rtwalwm_referee_aff){
				$rtwalwm_affiliate_id = $rtwalwm_referee_aff[0]->ID;
			}
	...
	function rtwalwm_generate_custom_code($rtwalwm_len){
		$rtwalwm_characters = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
		$rtwalwm_randomString = '';
		// $n = 6;
		...
		$rtwalwm_all_users = get_users(array(
			'meta_key'     => 'rtwalwm_referee_custom_str',
			'meta_value'   => $rtwalwm_randomString,
		));
		
		if($rtwalwm_all_users){
			$this->rtwalwm_generate_custom_code(6);
		}
		else{
			return $rtwalwm_randomString;
		}
